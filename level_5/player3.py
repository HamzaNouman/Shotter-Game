...
class Player3():
    ...
    def check_game_over(self):
        if self.health < 1:
            self.game.object_renderer3.game_over()
            pg.display.flip()
    ...
    def check_vict(self):
        if self.game.alive4 == False and self.game.alive5 == False:
            self.game.object_renderer3.vict()
            self.game.alive4 = True
            pg.display.flip()
            pg.time.delay(1500)
            self.game.new_game()
    ...
    def get_damage(self,damage):
        self.health -= damage
        self.game.object_renderer3.player_damage()
        self.game.sound3.player_pain.play()
        self.check_game_over()
        
    def single_fire_event(self, event):
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 1 and not self.shot and not self.game.weapon3.reloading:
                self.game.sound3.shotgun.play()
                self.shot=True
                self.game.weapon3.reloading = True
