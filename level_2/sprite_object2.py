...
class Sprite2():
    ...
    def get_sprite_proj(self):
        ...
        self.game.raycasting2.obj_to_reneder.append((self.norm_dist , image ,pos))
        
    def get_sprite(self):
        dx =self.x - self.player2.x
        dy=self.y - self.player2.y
        self.dx,self.dy = dx,dy
        self.theta = math.atan2(dy, dx)
        
        delta = self.theta - self.player2.angle
        if (dx > 0 and self.player2.angle >math.pi) or (dx<0 and dy < 0):
            delta += math.tau 
