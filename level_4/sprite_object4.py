...
class Sprite4():
    ...
    def get_sprite_proj(self):
        ...
        self.game.raycasting4.obj_to_reneder.append((self.norm_dist , image ,pos))
        
    def get_sprite(self):
        dx =self.x - self.player4.x
        dy=self.y - self.player4.y
        self.dx,self.dy = dx,dy
        self.theta = math.atan2(dy, dx)
        
        delta = self.theta - self.player4.angle
        if (dx > 0 and self.player4.angle >math.pi) or (dx<0 and dy < 0):
            delta += math.tau 
